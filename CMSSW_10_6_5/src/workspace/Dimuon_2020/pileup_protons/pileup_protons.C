...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   Long64_t nentries = fChain->GetEntriesFast();
	...
	TH1F* h_event_selection = new TH1F("event_selection", "", 4, 0, 4);
   ...
   for (Long64_t jentry=0; jentry<nentries ; jentry++) {
      ...
      if( has_pixel_23 ) h_event_selection->Fill(2);

      if( has_pixel_123 ) h_event_selection->Fill(3);    
   }

   output_file->Write();

   std::cout << "Total number of events: " << h_event_selection->GetBinContent(1) << std::endl
             << "Number of events with a candidate in RP23 or RP123: " << h_event_selection->GetBinContent(2) << std::endl
             << "Number of events with a candidate in RP23: " << h_event_selection->GetBinContent(3) << std::endl
             << "Number of events with a candidate in RP123: " << h_event_selection->GetBinContent(4) << std::endl;

}

int run() {
   pileup_protons m;
   m.SelectEvents(false);
   m.Loop();
