...
class ntp1 {
   public :
      ...
      ntp1(TTree *tree=0);
      ntp1(std::string const&, TTree *tree=0);
      virtual ~ntp1();
...
#ifdef ntp1_cxx
ntp1::ntp1(std::string const& fileName_, TTree *tree) : fChain(0), resample(false), resample_number(-1), fileName(fileName_)
{
...
ntp1::ntp1(TTree *tree) : ntp1("input.root", tree) {}
...
void ntp1::Init(TTree *tree)
{
   ...
   if( !mix_pileup ){
      ...
      mix_file = TFile::Open("/eos/user/m/malvesga/miniaod/pileup_protons_EG/pileup_protons_EG.root","READ");
